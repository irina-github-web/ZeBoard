  ...
  $.each(data, function (i, item) {
    ...
    $.each(item.videos, (iVideoId, aVideo) => {
      result_preview.push(
        ...
        data-tooltip-mouseover="${aVideo.video_name}" data-id=${iVideoId}></div>`
      );
    });
...
$("#vebirars").on("click", ".video_preview", function () {
  var previewVal = $(this).attr("data-id"); //выбранное название видео
  ...
  $.getJSON("https://video.gd.ru/event-type-2", function (data) {
    $.each(data, function (i, item) {
      $.each(item.videos, (iVideoId, aVideo) => {
        var videonameVal = iVideoId; //название видео в переменную, и ниже проверка на соответствие
