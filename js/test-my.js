  ...
  $.each(data, function (i, item) {
    ...
    $(".slider-for").slick(
      ...
      '<div class="container-upr">' +
        ...
        `<a href=${item.videos.video_url[1]} class="fancybox-media w-inline-block">` +
        `<div class="video" style="background-color:rgba(63, 145, 229, 0.7);background-image: url(${item.videos.video_pic_url[1]});background-size:cover;"><img src="images/icn_play_big.png" alt="" class="icn_big_play">` +
        '<div class="pl_blue"></div></div></a>' +
        '<div id="video_description" class="video_description">' +
        `<h2 class="heading_middle heading_video">${item.videos.video_name[1]}</h2>` +
        '<div class="razdelit_mid"></div>' +
        `<div class="txt_author">${item.videos.video_author[1]}</div>` +
